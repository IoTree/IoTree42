...
class MQTT_publisher:
    ...
    def __init__(self, broker='localhost', port=1883):
        ...
        self.mqtt_port = port
        self.mqtt_topic = 'sensorbase/gatewayself/Radon2'
        self.mqtt_deviceid = 'radon_02'
    ...
    def publish(self, value):
        ...
        now = int(time.time())
#        value = {"radon":value["radon"]}
        value = {"radon_[bq/m3]":value["radon"], "time":now, "debug":value["status"]}
        value = json.dumps(value)
        self.logger.debug("Sending value '{}' to MQTT broker".format(value))
        (result, mid) = self._client.publish(self.mqtt_topic, value, 2, retain=True)
